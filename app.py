...
def load_initial_data():
    global entries_storage, users_storage
    # Add a default user for testing
    users_storage.append({
        'id': '1',
        'username': os.environ.get('DEFAULT_USERNAME', 'testuser'),
        'email': os.environ.get('DEFAULT_EMAIL', 'test@example.com'),
        'password': os.environ.get('DEFAULT_PASSWORD', 'password123'),  # In real app, store hashed password
        'created_at': datetime.datetime.utcnow().isoformat()
    })
    
    # Check for 'dami' account preference
    if os.environ.get('CREATE_DAMI_ACCOUNT', 'false').lower() == 'true':
        users_storage.append({
            'id': '2',
            'username': 'dami',
            'email': 'dami@example.com',
            'password': 'damipassword123',  # In real app, store hashed password
            'created_at': datetime.datetime.utcnow().isoformat()
        })
...
def get_entries():
    ...
    return jsonify({"entries": list(reversed(entries_storage))})
...
def add_entry():
    ...
    entries_storage.append(new_entry)
    
    # Return entries in reverse order (newest first)
    return jsonify({"message": "Entry added successfully!", "entries": list(reversed(entries_storage))})

@app.route('/get_sentiment', methods=['GET'])
def get_sentiment():
    if not entries_storage:
        return jsonify({"message": "No entries available for sentiment analysis."})

    all_entries = " ".join([entry['entry'] for entry in entries_storage])
...
def get_daily_sentiment():
    ...
    today_entries = [entry for entry in entries_storage if entry['date'] == today_str]
    
    if not today_entries:
        return jsonify({"message": "No entries available for today."})
    
    all_entries = " ".join([entry['entry'] for entry in today_entries])
    sentiment_score = sia.polarity_scores(all_entries)
...
def get_monthly_sentiment():
    ...
    # Filter entries for the past month
    filtered_entries = []
    for entry in entries_storage:
        try:
            entry_date_str = entry['date'].split(", ")[1]  # Get "YYYY-MM-DD" part
